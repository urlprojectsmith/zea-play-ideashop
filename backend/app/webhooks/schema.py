...
def ensure_webhook_schema() -> None:
    ...
    try:
        with engine.begin() as connection:
            # Avoid indefinite waits when another session holds locks on webhook tables.
            if dialect == "postgresql":
        ...
        logger.warning("Skipping webhook schema sync for now due to database error: %s", exc)
