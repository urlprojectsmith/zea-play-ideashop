...
def _get_admin_owner_ids(db: Session, tenant_id: Optional[Any]) -> set[str]:
    stmt = select(models.User.id).where(models.User.role.in_([models.RoleEnum.ADMIN, models.RoleEnum.OWNER]))
    if tenant_id:
...
def _get_task_oversight_recipient_ids(db: Session, assignee: Optional[models.User], *exclude_ids: Optional[str]) -> set[str]:
    tenant_id = assignee.tenant_id if assignee else None
...
def list_tasks(
    ...
    if not skip_overdue_notifications:
        if current_user.role in {models.RoleEnum.ADMIN, models.RoleEnum.OWNER, models.RoleEnum.MANAGER}:
            overdue_tasks = [
                task
                for task in tasks
        ...
        if overdue_tasks:
            ...
            assignee_ids = {str(task.assigned_to_id) for task in overdue_tasks if task.assigned_to_id}
            ...
            for task in overdue_tasks:
                if has_overdue_notified:
                    claimed = db.execute(
                        text(
                            ...
                            WHERE id = :task_id AND overdue_notified = 0
                            """
                        ),
                        {"task_id": task.id},
                    )
                ...
                assignee_notification_created = False

                if assignee_id and (assignee_id, task.id) not in existing_overdue_by_assignee:
                    ...
                    assignee_notification_created = True
                    existing_overdue_by_assignee.add((assignee_id, task.id))

                if assignee_notification_created and assignee_id:
                    assignee = task.assignee if task.assignee and str(task.assignee.id) == assignee_id else db.get(models.User, assignee_id)
                    assignee_name = assignee.name if assignee and assignee.name else "Assigned user"
                    browser_push_service.send_to_user(
                        db,
                        user_id=assignee_id,
                        title="Task overdue",
                    ...
                    if oversight_ids:
                        browser_push_service.send_to_users(
                            db,
    ...
    if auto_approve:
        if payload.to_user_id != current_user.id:
            browser_push_service.send_to_user(
                ...
                body=f"A task was transferred to you: {task.title}",
                related_task_id=task.id,
                deep_link=f"/tasks/{task.id}",
                tag=f"task-transfer-assigned-{task.id}",
                event_type="task.transfer.assigned",
            )
        ...
        if recipient_ids:
            browser_push_service.send_to_users(
                ...
                body=f"{current_user.name} requested transfer for task: {task.title}",
                related_task_id=task.id,
                deep_link=f"/tasks/{task.id}",
                tag=f"task-transfer-request-{task.id}",
                event_type="task.transfer.requested",
            )
        db.commit()
...
def act_on_transfer_request(
    ...
    browser_push_service.send_to_user(
        db,
        user_id=str(transfer_request.requested_by_id),
        title="Task transfer decision",
        body=f"Your transfer request was {payload.decision}: {task.title}",
        related_task_id=task.id,
        deep_link=f"/tasks/{task.id}",
        tag=f"task-transfer-{payload.decision}-{task.id}",
        event_type="task.transfer.decision",
    )
    if payload.decision == "approved" and transfer_request.to_user_id != current_user.id:
        browser_push_service.send_to_user(
            db,
            user_id=str(transfer_request.to_user_id),
            title="Task transferred to you",
            body=f"A task was transferred to you: {task.title}",
            related_task_id=task.id,
            deep_link=f"/tasks/{task.id}",
...
def list_tasks_kanban(
    ...
    counts_rows = db.execute(
        ...
        .select_from(base_stmt.order_by(None).subquery())
...
def create_task(
    ...
    normalized_due_at = _normalize_utc_naive(payload.due_at)
    ...
    current_user.points += 10 * created_count
    ...
    for task in created_tasks:
        ...
        # Task creation notifications must target only the assignee and never the creator.
        should_notify_assignee = bool(
            assignee
            and task.assigned_to_id
            and str(task.assigned_to_id) != str(current_user.id)
        )
        if should_notify_assignee and assignee:
            notification_service.create_notification(
                db,
                user_id=assignee.id,
                actor_id=str(current_user.id),
            ...
            browser_push_service.send_to_user(
                ...
                body=f"You have a new task: {task.title}",
                related_task_id=task.id,
                deep_link=f"/tasks/{task.id}",
                tag=f"task-created-{task.id}",
                event_type="task.created",
            )
...
def update_task(
    ...
    if "due_at" in update_data:
        task.due_at = _normalize_utc_naive(update_data["due_at"])
        if has_overdue_notified and (task.due_at is None or task.due_at > datetime.utcnow()):
            if has_overdue_notified_at:
                db.execute(
                    text("UPDATE tasks SET overdue_notified = 0, overdue_notified_at = NULL WHERE id = :task_id"),
                    {"task_id": task.id},
                )
            else:
                db.execute(
                    text("UPDATE tasks SET overdue_notified = 0 WHERE id = :task_id"),
                    {"task_id": task.id},
                )
    if "completed_at" in update_data:
        task.completed_at = _normalize_utc_naive(update_data["completed_at"])
    ...
    if assignee_notification_target and assignee_notification_target.id != current_user.id:
        ...
        browser_push_service.send_to_user(
            ...
            user_id=assignee_notification_target.id,
            title="Task assigned",
            body=f"You have a new task assignment: {task.title}",
            related_task_id=task.id,
            deep_link=f"/tasks/{task.id}",
            tag=f"task-assigned-{task.id}",
            event_type="task.assigned",
        )

        oversight_ids = _get_task_oversight_recipient_ids(
            db,
            assignee_notification_target,
            current_user.id,
            assignee_notification_target.id,
        )
        if oversight_ids:
            browser_push_service.send_to_users(
                db,
                ...
                body=f"{assignee_notification_target.name} was assigned: {task.title}",
                related_task_id=task.id,
                deep_link=f"/tasks/{task.id}",
                tag=f"task-assigned-team-{task.id}",
                event_type="task.assigned.team",
            )
    ...
    if should_award_points:
        ...
        assignee_user = db.get(models.User, task.assigned_to_id) if task.assigned_to_id else None
        oversight_ids = _get_task_oversight_recipient_ids(
            db,
            assignee_user,
            current_user.id,
            assignee_user.id if assignee_user else None,
        )
        completed_by_name = assignee_user.name if assignee_user else current_user.name
        if oversight_ids:
            browser_push_service.send_to_users(
                db,
                user_ids=oversight_ids,
                title="Task completed",
                body=f"{completed_by_name} completed task: {task.title}",
                related_task_id=task.id,
                deep_link=f"/tasks/{task.id}",
                tag=f"task-completed-{task.id}",
        ...
        if current_user.tenant_id:
            ...
            except SQLAlchemyError as exc:  # pragma: no cover - best effort reporting
                logger.warning(
                    "Unable to record reporting events for task %s: %s", task.id, exc
                )
        assignee_user = db.get(models.User, task.assigned_to_id) if task.assigned_to_id else None
        oversight_ids = _get_task_oversight_recipient_ids(
            db,
