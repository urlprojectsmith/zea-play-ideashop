...
from .services.overdue_tasks import start_overdue_task_worker, stop_overdue_task_worker
...
def ensure_pg_trgm_extension() -> None:
    ...
    try:
        ...
        logger.warning("Unable to ensure pg_trgm extension: %s", exc)


def _disable_trgm_indexes_in_metadata() -> None:
...
def startup_event() -> None:
    def _run_startup_step(name: str, fn) -> None:
        print(f"Startup step started: {name}", flush=True)
        logger.info("Startup step started: %s", name)
        try:
            fn()
        except Exception:
            print(f"Startup step failed: {name}", flush=True)
            logger.exception("Startup step failed: %s", name)
            raise
        print(f"Startup step completed: {name}", flush=True)
    ...
    def _create_tables() -> None:
        try:
            ...
            if "gin_trgm_ops" in message or "pg_trgm" in message:
                logger.warning(
                    "Skipping trigram index creation because pg_trgm is unavailable: %s",
                    exc,
                )
                _disable_trgm_indexes_in_metadata()
                Base.metadata.create_all(bind=engine)
            else:
                raise

    _run_startup_step("ensure_task_status_enum", ensure_task_status_enum)
    _run_startup_step("ensure_pg_trgm_extension", ensure_pg_trgm_extension)
    _run_startup_step("ensure_reward_schema", ensure_reward_schema)
    _run_startup_step("create_all_tables", _create_tables)
    _run_startup_step("ensure_employer_id_column", ensure_employer_id_column)
    _run_startup_step("ensure_user_profile_columns", ensure_user_profile_columns)
    _run_startup_step("ensure_media_file_columns", ensure_media_file_columns)
    if settings.webhook_schema_sync_on_startup:
        _run_startup_step("ensure_webhook_schema", ensure_webhook_schema)
    else:
        print("Startup step skipped: ensure_webhook_schema", flush=True)
        logger.info("Startup step skipped: ensure_webhook_schema")
    _run_startup_step("seed_database", seed_database)
    _run_startup_step("seed_badges_from_achievements", seed_badges_from_achievements)
    _run_startup_step("ensure_kanban_columns", ensure_kanban_columns)
    _run_startup_step("ensure_avatar_dirs", ensure_avatar_dirs)
    _run_startup_step("start_webhook_retry_worker", start_webhook_retry_worker)
    _run_startup_step("start_overdue_task_worker", start_overdue_task_worker)

    def _start_task_event_listener() -> None:
        loop = asyncio.get_event_loop()
        task_events.start_task_event_listener(loop=loop, handler=tasks_ws.broadcast_task_event)

    _run_startup_step("start_task_event_listener", _start_task_event_listener)


@app.on_event("shutdown")
def shutdown_event() -> None:
    stop_webhook_retry_worker()
    stop_overdue_task_worker()
...
app.include_router(browser_push.router)
...
app.include_router(browser_push.router, prefix="/api")
