...
class UserBase(BaseModel):
    ...
    browser_notifications_enabled: Optional[bool] = True
    is_present: Optional[int] = 0
...
class BrowserPushSubscriptionCreate(BaseModel):
    endpoint: str
    keys: BrowserPushKeys


class BrowserPushSubscriptionRead(BaseModel):
    model_config = ConfigDict(from_attributes=True)
...
class BrowserPushSettingsRead(BaseModel):
    enabled: bool
    vapid_public_key: Optional[str] = None
    subscriptions: List[BrowserPushSubscriptionRead] = Field(default_factory=list)


class BrowserPushSettingsUpdate(BaseModel):
    enabled: bool
