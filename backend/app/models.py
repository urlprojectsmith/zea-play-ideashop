...
class User(Base):
    ...
    browser_notifications_enabled: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    is_present: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    ...
    browser_push_subscriptions: Mapped[List["BrowserPushSubscription"]] = relationship(
...
class RewardClaim(Base):
    ...
    __table_args__ = (
        UniqueConstraint("reward_id", "user_id", name="uq_reward_claim"),
    )
    ...
    claimed_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
...
class BrowserPushSubscription(Base):
    ...
    user: Mapped["User"] = relationship("User", back_populates="browser_push_subscriptions")
