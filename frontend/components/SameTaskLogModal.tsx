        ...
        <div className="p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-white">Track every teammate working on this linked clinician task.</p>
            </div>
                    ...
                    <div key={task.id} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/30 px-3 py-3">
                      ...
                      <div className="flex flex-1 flex-col">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs text-white/80">{timeAgo(task.updatedAt)}</span>
                        </div>
                ...
                {activityFeed.length === 0 ? (
                  ...
                  <ul className="space-y-3">
                    {activityFeed.slice(0, 8).map((entry) => {
                      ...
                      return (
                        <li key={entry.id} className="rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white">
                          <div className="flex items-center justify-between">
                            <span className="font-semibold">{actor?.name ?? 'System update'}</span>
                            <span className="text-xs text-white/80">{formatDate(entry.updatedAt, true)}</span>
                          </div>
                          <p className="mt-1 text-xs text-white">
                            Status synced to <span className="font-semibold text-sky-200">{formatTaskStatus(entry.status)}</span> on “{entry.title}”.
                          </p>
                        </li>
                      );
                    })}
