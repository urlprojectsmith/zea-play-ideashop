...
const GenerateTaskWithAIModal: React.FC<GenerateTaskWithAIModalProps> = ({ isOpen, onClose, onTaskGenerated }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 z-[60] flex justify-center items-center p-4" onClick={handleOverlayClick}>
            <div className="bg-[var(--modal-surface-bg,white)] p-8 rounded-lg shadow-xl w-full max-w-lg border max-h-[85vh] overflow-y-auto custom-scrollbar" style={{ borderColor: 'var(--modal-border, rgba(0,0,0,0.1))' }}>
                <h2 className="text-2xl font-bold" style={{ color: 'var(--modal-text,#000)' }}>Generate Task with AI</h2>
                <p className="mt-1 mb-6" style={{ color: 'var(--modal-muted,#000)' }}>Simply describe what you need, and AI will draft the task for you.</p>

                <div className="space-y-4">
                    <div>
                        <p className="text-sm font-bold uppercase tracking-wider" style={{ color: 'var(--modal-muted,#000)' }}>Feature prompt style</p>
                        <div className="mt-2 grid grid-cols-2 gap-2">
                            {Object.entries(PROMPT_MODES).map(([key, mode]) => (
                                <button
                                    ...
                                    className={`rounded-md border px-3 py-2 text-left text-sm transition ${promptMode === key
                                            ? 'bg-black/5'
                                            : 'bg-transparent border-black/10 hover:border-black/30'
                                        }`}
                                    style={{
                                        color: 'var(--modal-text,#000)',
                                        borderColor: promptMode === key ? 'var(--modal-accent)' : undefined
                                    }}
                                >
                        ...
                        <p className="mt-2 text-xs" style={{ color: 'var(--modal-muted,#000)' }}>
                            Detail mode provides step-by-step guidance, a detailed explanation, and subtasks.
                        </p>
                    </div>

                    <div>
                        <label htmlFor="ai-prompt" className="block text-sm font-bold uppercase tracking-wider" style={{ color: 'var(--modal-muted,#000)' }}>Type Your Prompt</label>
                        <textarea
                            id="ai-prompt"
                            rows={5}
                            value={prompt}
                            ...
                            className="mt-1 block w-full bg-white border border-black/10 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-1 focus:ring-black"
                            style={{ color: 'var(--modal-text,#000)' }}
                            disabled={isQuestionStep}
                        />
                        <p className="text-xs mt-2 flex items-center gap-2" style={{ color: 'var(--modal-muted,#000)' }}>
                            <SparklesIcon className="h-4 w-4" style={{ color: 'var(--modal-accent)' }} />
                            Be specific about details like purpose, audience, and desired outcome to get the best results.
                        </p>
                    </div>

                    <label className="flex items-center gap-2 text-sm" style={{ color: 'var(--modal-text,#000)' }}>
                        <input
                            type="checkbox"
                    ...
                    {askQuestions && !isQuestionStep && (
                        <p className="text-xs" style={{ color: 'var(--modal-muted,#000)' }}>
                    ...
                    {isQuestionStep && (
                        <div className="rounded-md border border-black/10 bg-black/5 p-3">
                            <div className="flex items-center justify-between gap-2">
                                <p className="text-sm font-bold" style={{ color: 'var(--modal-text,#000)' }}>Answer these questions</p>
                                <button
                                    type="button"
                                    onClick={() => {
                                        setQuestions([]);
                            ...
                            <div className="mt-3 space-y-3">
                                {questions.map((question, index) => (
                                    <label key={`${question}-${index}`} className="block text-xs font-bold" style={{ color: 'var(--modal-muted,#000)' }}>
                                        {question}
                                        <input
                                            type="text"
                                            value={answers[index] ?? ''}
                                            ...
                                            className="mt-1 w-full rounded-md border border-black/10 bg-white px-3 py-2 text-sm focus:outline-none"
                                            style={{ color: 'var(--modal-text,#000)' }}
                                        />
                                    </label>
                                ))}
                ...
                <div className="flex justify-end space-x-4 pt-6">
                    <button type="button" onClick={onClose} className="py-2 px-6 bg-slate-100 text-black border border-black/10 rounded-md font-bold text-sm">
                        Cancel
                    </button>
                    <button
                        type="button"
                        ...
                        className="py-2 px-6 bg-black text-white rounded-md flex items-center space-x-2 font-bold text-sm disabled:opacity-30"
                    >
