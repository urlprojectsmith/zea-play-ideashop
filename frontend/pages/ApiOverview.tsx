    ...
    },
    {
        ...
        endpoints: [
            ...
            },
            {
                ...
                method: 'POST',
                path: '/api/tickets/tasks/{id}/complete',
...
const ApiOverview: React.FC = () => {
    ...
    const API_OVERVIEW_ACCESS_PASSWORD = 'MuhZt2C#c@4%dD';
    const API_OVERVIEW_UNLOCK_KEY = 'zea-play::api-overview-unlocked';
    ...
    const [isAccessGranted, setIsAccessGranted] = useState(() => {
        ...
        try {
            return window.sessionStorage?.getItem(API_OVERVIEW_UNLOCK_KEY) === 'granted';
        } catch {
            return false;
        }
    });
    ...
    const handleAccessSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (passwordInput === API_OVERVIEW_ACCESS_PASSWORD) {
            setIsAccessGranted(true);
            setPasswordInput('');
            setAuthError(null);
            if (typeof window !== 'undefined' && 'sessionStorage' in window) {
                try {
                    window.sessionStorage.setItem(API_OVERVIEW_UNLOCK_KEY, 'granted');
                } catch {
    ...
    useEffect(() => {
        if (!isAccessGranted || typeof window === 'undefined') {
            return;
        }
        window.localStorage.setItem(KEY_HISTORY_STORAGE_KEY, JSON.stringify(keyHistory));
    }, [keyHistory, isAccessGranted]);

    useEffect(() => {
        if (!isAccessGranted || hasHydratedFromHistory || keyHistory.length === 0) {
            return;
        }
    ...
    }, [keyHistory, hasHydratedFromHistory, isAccessGranted]);
    ...
    useEffect(() => {
        if (!isAccessGranted || activeTab !== 'webhooks') {
            return;
        }
        refreshWebhooks();
    ...
    if (!isAccessGranted) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-900 px-4 py-8">
                <div className="w-full max-w-md rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900 via-slate-900/90 to-slate-800 p-8 text-white shadow-2xl">
                    <p className="text-xs uppercase tracking-[0.4em] text-cyan-300">System Integration</p>
                    <h2 className="mt-3 text-2xl font-semibold">Secure API Access</h2>
                    <p className="mt-2 text-sm text-white/70">
                        Enter the shared password to unlock the API Overview tab. The password is only kept in session memory for this browser tab.
                    </p>
                    <form onSubmit={handleAccessSubmit} className="mt-6 space-y-4">
                        <label className="block text-sm font-medium text-white/80" htmlFor="api-access-password">
                            Access password
                        </label>
                        <input
                            id="api-access-password"
                            type="password"
                            value={passwordInput}
                            onChange={handlePasswordChange}
                            className="w-full rounded-2xl border border-white/30 bg-white/5 px-4 py-3 text-sm text-white placeholder:text-white/60 focus:border-white focus:outline-none focus:ring-2 focus:ring-white/70"
                            placeholder="Enter password"
                        />
                        {authError && <p className="text-xs text-rose-300">{authError}</p>}
                        <button
                            type="submit"
                            className="w-full rounded-2xl bg-gradient-to-r from-teal-500 to-cyan-500 px-4 py-3 text-sm font-semibold uppercase tracking-[0.3em] text-white transition hover:brightness-110"
                        >
                            Unlock API Overview
                        </button>
                    </form>
