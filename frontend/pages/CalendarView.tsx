...
import { PlusIcon, DocumentTextIcon, BellIcon, XMarkIcon, SparklesIcon, FunnelIcon } from '../components/icons';
...
const CalendarView: React.FC = () => {
            ...
            .map((task) => {
                ...
                return (
                    <div
                        ...
                        className={`group w-full rounded-md border border-rose-200/60 bg-white/80 p-2 text-left text-xs sm:text-sm text-gray-800 shadow-none transition hover:bg-white ${resolvedTheme === 'professional' ? 'task-card-professional' : 'task-card-gamified'
                            }`}
                    >
                        <div className="flex items-center justify-between gap-2">
                            <p className="flex-1 break-words font-medium text-gray-800 line-clamp-2">{task.title}</p>
                            <TaskPriorityBadge priority={task.priority} />
                        </div>
                        <div className="mt-2 flex items-center gap-2 text-xs text-gray-500">
                            <div className="h-6 w-6 rounded-full border border-rose-200/60 bg-rose-50 text-center text-[10px] font-semibold leading-6 text-gray-600">
                                {(() => {
                            ...
                            <span>{CUSTOM_STATUS_NAMES[task.status]?.name || task.status.replace(/_/g, ' ').toLowerCase()}</span>
                        ...
                        {progress !== null && (
                            <div className="mt-2">
                                ...
                                <p className="mt-1 text-[10px] text-gray-500">Progress {progress}%</p>
                            </div>
                        )}
                    </div>
                );
    ...
    const renderCells = () => {
        ...
        daysInView.forEach((day, index) => {
            ...
            const dateBadgeBase = `ml-auto flex h-5 w-5 sm:h-7 sm:w-7 items-center justify-center rounded-full border text-[10px] sm:text-sm font-semibold ${dateKey === todayKey ? 'border-indigo-500 bg-indigo-100 text-indigo-800' : 'border-gray-200 bg-white text-gray-700'
                }`;
            const quickActionButtonStyles = isDark
                ? 'border-gray-200 bg-white text-gray-700 hover:bg-gray-50'
                : isColorful
                    ? 'border-gray-200 bg-white text-gray-700 hover:bg-gray-50'
                    : 'border-slate-200 bg-white text-black hover:border-indigo-300 hover:text-black';
            const quickActionPanelSurface = isDark
                ? 'border-gray-200 bg-white text-gray-700 shadow-sm'
                : isColorful
                    ? 'border-gray-200 bg-white text-gray-700 shadow-sm'
                    : 'border-gray-200 bg-white text-gray-700 shadow-sm';
            const metaTextClass = 'text-gray-500';
            const metaLinkClass = isDark
                ? 'text-gray-600 hover:text-gray-700 hover:bg-gray-50'
                : isColorful
                    ? 'text-gray-600 hover:text-gray-700 hover:bg-gray-50'
                    : 'text-gray-600 hover:text-gray-700 hover:bg-indigo-50';
            const quickActionOptionClass = isDark
                ? 'flex w-full items-center gap-2 rounded-md border border-gray-200 px-2 py-2 text-left hover:bg-gray-50'
                : isColorful
                    ? 'flex w-full items-center gap-2 rounded-md border border-gray-200 px-2 py-2 text-left hover:bg-gray-50'
                    : 'flex w-full items-center gap-2 rounded-md border border-gray-200 px-2 py-2 text-left hover:border-indigo-200 hover:bg-indigo-50';

            const cell = (
                <div
                    key={dateKey}
                    className={`relative flex h-full min-h-[88px] sm:min-h-[120px] md:min-h-[160px] flex-col gap-1 sm:gap-2 rounded-lg border border-gray-200 p-2 sm:p-3 lg:p-4 transition ${inCurrentMonth ? 'bg-white hover:bg-gray-50' : 'bg-gray-50 text-gray-500'
                        } ${isConflict ? 'ring-2 ring-rose-400/60' : ''} ${isQuickActionsOpen ? 'z-[55]' : 'z-auto'} ${resolvedTheme === 'professional' ? 'calendar-cell-professional' : 'calendar-cell-gamified'
                        }`}
                ...
                >
                    <div className="flex items-center justify-between text-[10px] sm:text-xs">
                        <button
                            ...
                            className={`flex h-5 w-5 sm:h-6 sm:w-6 md:h-7 md:w-7 items-center justify-center rounded-full border text-[10px] sm:text-[12px] md:text-[13px] transition ${quickActionButtonStyles}`}
                    ...
                    {isQuickActionsOpen && (
                        <div
                            className={`absolute right-2 top-10 md:right-3 md:top-12 z-[60] w-44 md:w-48 rounded-lg border p-3 text-xs ${quickActionPanelSurface}`}
                    ...
                    <div className="flex-1 space-y-1 sm:space-y-2 overflow-y-auto pr-0.5 sm:pr-1">
            ...
            if (isMonth && (index + 1) % 7 === 0) {
                rows.push(
                    <div key={`row-${dateKey}`} className="grid w-full min-w-0 grid-cols-3 min-[480px]:grid-cols-4 md:grid-cols-7 gap-2 md:gap-3 lg:gap-4">
                        {cells.splice(0)}
                    </div>
                );
            }
        ...
        if (viewMode === 'month') {
            return <div className="space-y-1 sm:space-y-3">{rows}</div>;
        }

        if (viewMode === 'week') {
            return <div className="grid w-full min-w-0 grid-cols-3 min-[480px]:grid-cols-4 md:grid-cols-7 gap-2 md:gap-3 lg:gap-4">{cells}</div>;
        }

        return <div className="grid gap-2 sm:gap-3 lg:gap-4">{cells}</div>;
    };

    const quickActionModalSurface = isDark
        ? 'bg-slate-900/90 border-white/15 text-black'
        : isColorful
            ? 'bg-white/95 border-white/60 text-black shadow-xl'
            : 'bg-white border-slate-200 text-black shadow-xl';
    const modalSecondaryButton = isDark
        ? 'rounded-full border border-white/20 px-3 py-1 text-xs text-black hover:border-white/40 hover:text-black'
        : 'rounded-full border border-slate-200 px-3 py-1 text-xs text-black hover:border-indigo-200 hover:text-black';
    const modalPrimaryButton = 'rounded-full bg-gradient-to-r from-sky-400 to-violet-400 px-3 py-1 text-xs font-semibold text-black shadow hover:brightness-105 disabled:opacity-50 disabled:cursor-not-allowed';
    const modalSubtleText = isDark ? 'text-black' : 'text-black';
    const modalFieldClass = isDark
        ? 'border-white/20 bg-white/5 text-black placeholder:text-white/40 focus:border-sky-300'
        : isColorful
            ? 'border-white/60 bg-white text-black placeholder:text-black focus:border-violet-300'
            : 'border-slate-200 bg-white text-black placeholder:text-black focus:border-indigo-300';
    const modalLabelClass = isDark ? 'text-black' : 'text-black';
    const modalListCardClass = isDark
        ? 'border-white/15 bg-white/5 text-white/90'
        : isColorful
            ? 'border-white/60 bg-white text-black'
            : 'border-slate-200 bg-white text-black';
    const modalTimestampClass = isDark ? 'text-black' : 'text-black';
    const modalDangerButton = isDark
        ? 'rounded-full border border-red-400/40 px-3 py-1 text-xs text-black transition hover:border-red-300 hover:text-black hover:bg-red-500/10'
        : 'rounded-full border border-red-200 px-3 py-1 text-xs text-black transition hover:bg-red-50';
    ...
    if (loading) {
        return <div className="p-10 text-center text-black">Loading calendar</div>;
    }

    const topBarSurface = 'bg-white border-gray-200 text-gray-700 shadow-none';

    const buttonBase = 'rounded-md border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50';
    const activeBtn = 'border-indigo-500 bg-indigo-50 text-indigo-800';
    const inactiveBtn = 'border-gray-200 bg-white text-gray-700 hover:bg-gray-50 hover:border-gray-300';

    const controls = (
        <div className={`mb-6 flex flex-col gap-4 rounded-md border px-4 sm:px-5 py-4 md:flex-row md:flex-wrap md:items-center md:justify-between ${topBarSurface}`}>
            ...
            <div className="flex w-full flex-wrap items-center gap-2 text-xs md:w-auto">
                ...
                <span className="text-sm font-semibold text-gray-800">{formatRangeLabel()}</span>
            </div>
            <div className="md:hidden w-full mt-1">
                <button
                    onClick={() => setIsMobileFiltersOpen(!isMobileFiltersOpen)}
                    className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-md border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
                >
    ...
    const filterBar = (
        <div className={`mb-6 grid grid-cols-1 gap-4 rounded-md border px-4 sm:px-5 py-4 sm:grid-cols-2 lg:grid-cols-4 ${topBarSurface} ${!isMobileFiltersOpen ? 'hidden md:grid' : 'grid'}`}>
            <input
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                placeholder="Search tasks..."
                className="h-10 w-full rounded-md border border-gray-200 bg-white px-4 py-2 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-0 focus:border-indigo-500"
            />
            <select
                value={priorityFilter}
                onChange={(e) => setPriorityFilter(e.target.value as 'ALL' | TaskPriority)}
                className="h-10 w-full rounded-md border border-gray-200 bg-white px-4 py-2 text-sm text-gray-700 focus:outline-none focus:ring-0 focus:border-indigo-500"
            >
    ...
    const legend = (
        <div className="mb-6 flex flex-wrap items-center gap-4 text-xs text-gray-600">
            ...
            <span className="inline-flex items-center gap-2">
                <span className="h-3 w-3 rounded-full bg-white ring-2 ring-rose-400/60" />
                Conflict alert
            </span>
        </div>
    );
    ...
    return (
        <div className="space-y-6 px-3 sm:px-6 lg:px-8 overflow-x-hidden">
            <h1 className="mb-6 text-xl sm:text-2xl lg:text-3xl font-semibold tracking-tight text-gray-800">Calendar</h1>
            {controls}
            {filterBar}
            {legend}
            {viewMode === 'month' && (
                <div className="grid w-full min-w-0 grid-cols-3 min-[480px]:grid-cols-4 md:grid-cols-7 gap-2 md:gap-3 lg:gap-4 text-[10px] sm:text-sm font-medium uppercase tracking-[0.06em] sm:tracking-[0.15em] text-gray-600">
                    {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
                        <div key={day} className="rounded-md border border-gray-200 bg-white py-1.5 sm:py-2 text-center">
                            {day}
                        </div>
                    ))}
                </div>
            ...
            {renderCells()}
            <div className="rounded-md border border-dashed border-gray-200 bg-white p-4 text-xs text-gray-500">
            </div>
            {selectedTaskId && (
                <TaskDetailModal
