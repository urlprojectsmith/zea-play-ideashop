...
import { disableBrowserPushSubscription, isBrowserPushSupported, syncBrowserPushSubscription } from '../services/browserPush';
import {
    AvatarCropMetadata,
    DataExportScope,
    DataImportPayload,
...
const Settings: React.FC = () => {
    ...
    const [pushStatus, setPushStatus] = useState<StatusMessage>(null);
    const [pushSaving, setPushSaving] = useState(false);
    const [pushTesting, setPushTesting] = useState(false);
    const [browserPushEnabled, setBrowserPushEnabled] = useState(true);
    const [pushKeyConfigured, setPushKeyConfigured] = useState<boolean | null>(null);
    ...
    useEffect(() => {
        setBrowserPushEnabled(user?.browserNotificationsEnabled ?? true);
    }, [user?.browserNotificationsEnabled, user?.id]);
    ...
    const canManageBrowserNotifications = useMemo(
        () => user.role === Role.ADMIN || user.role === Role.OWNER || user.role === Role.MANAGER,
        [user.role],
    );
    ...
    const refreshPushDebug = async () => {
        ...
        try {
            ...
            setBrowserPushEnabled(Boolean(settings.enabled));
            setPushKeyConfigured(Boolean(settings.vapidPublicKey));
            setPushSubscriptionCount(settings.subscriptions?.length ?? 0);
        } catch (error) {
            console.error('Failed to refresh push debug info', error);
            setPushKeyConfigured(null);
    ...
    useEffect(() => {
        if (!canManageBrowserNotifications) {
            return;
        }
        void refreshPushDebug();
    }, [user?.id, canManageBrowserNotifications]);

    const handleBrowserPushToggle = async (nextEnabled: boolean) => {
        if (!user || !canManageBrowserNotifications) {
            return;
        }
        setPushSaving(true);
        setPushStatus(null);
        try {
            const updatedSettings = await api.updateBrowserPushSettings(nextEnabled);
            if (nextEnabled) {
                if (!isBrowserPushSupported()) {
                    throw new Error('Browser push is not supported on this browser.');
                }
                if (!updatedSettings.vapidPublicKey) {
                    throw new Error('Push key is not configured on the server. Ask admin to set VEE_VAPID keys.');
                }
                await syncBrowserPushSubscription({ ...user, browserNotificationsEnabled: true });
            } else {
                await disableBrowserPushSubscription();
            }
            setBrowserPushEnabled(nextEnabled);
            setPushKeyConfigured(Boolean(updatedSettings.vapidPublicKey));
            setPushSubscriptionCount(updatedSettings.subscriptions?.length ?? 0);
            updateUserInContext({ ...user, browserNotificationsEnabled: nextEnabled });
            setPushStatus({
                type: 'success',
                message: nextEnabled
            ...
            setPushStatus({
                ...
                message: error instanceof Error
                    ? error.message
                    : 'Unable to update browser notification preference.',
            });
        } finally {
            setPushSaving(false);
        }
    ...
    const handlePushTest = async () => {
        ...
        setPushStatus(null);
        try {
            console.log('Settings.tsx: Starting push test synchronization...');
            // Ensure current browser has an active subscription for this user before test send.
            await syncBrowserPushSubscription(user);
            console.log('Settings.tsx: Synchronization finished. Sending test push...');
            const result = await api.sendBrowserPushTest();
            console.log('Settings.tsx: Test push result:', result);
            await refreshPushDebug();
            setPushStatus({
                type: result.sentCount > 0 ? 'success' : 'error',
                message: result.sentCount > 0
                    ? 'Test push sent. Check browser popup and sound.'
                    : 'Subscription not active for this browser/user. Try toggling browser push OFF then ON and retry.',
            });
        } catch (error) {
            console.error('Settings.tsx: Error during push test', error);
            setPushStatus({
                type: 'error',
                message: error instanceof Error ? error.message : 'Failed to send test push.',
            });
        } finally {
                    ...
                    <div className="flex gap-2">
                        {TABS.map(({ key, label, icon: Icon, description }) => (
                            <button
                                ...
                                className={`flex items-center gap-2 px-4 py-2 rounded-lg border ${activeTab === key
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                        <div className="bg-surface border border-border-color rounded-xl p-6 space-y-5 shadow-sm">
                            ...
                            {canManageBrowserNotifications && (
                                <div className="rounded-lg border border-border-color/70 p-4 space-y-3">
                                    <div>
                                        <p className="text-sm font-semibold text-text-primary">Browser notifications</p>
                                        <p className="text-xs text-text-secondary">
                                            Enable push notifications for task assigned, completed, overdue, and transfer events.
                                        </p>
                                    </div>
                                    {pushStatus && <StatusBanner status={pushStatus} />}
                                    <div className="flex items-center gap-3">
                                        <button
                                            type="button"
                                            onClick={() => handleBrowserPushToggle(!browserPushEnabled)}
                                            disabled={pushSaving}
                                            className={`inline-flex items-center justify-center px-4 py-2 rounded-lg text-sm font-medium transition ${browserPushEnabled
                                        ...
                                        <span className="text-xs text-text-secondary">
                                            Status: {browserPushEnabled ? 'Enabled' : 'Disabled'}
                                        </span>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <button
                                            ...
                                            disabled={pushTesting || !browserPushEnabled}
                                            className="inline-flex items-center justify-center px-4 py-2 rounded-lg text-sm font-medium bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition"
                                        >
                                            {pushTesting ? 'Sending test...' : 'Send test notification'}
                                        </button>
                                        <span className="text-xs text-text-secondary">
                                            Sends test push to this logged-in browser.
                                        </span>
                                    </div>
                                    <div className="rounded-md border border-border-color/60 p-3 text-xs text-text-secondary space-y-2">
                                        <div className="flex items-center justify-between">
                                            <span>VAPID key configured</span>
                                            <span className="font-semibold text-text-primary">
                                                {pushDebugLoading ? 'Checking...' : pushKeyConfigured ? 'Yes' : 'No'}
                                            </span>
                                        </div>
                                        <div className="flex items-center justify-between">
                                            <span>Active subscriptions (this user)</span>
                                            <span className="font-semibold text-text-primary">
                                                {pushDebugLoading ? '...' : pushSubscriptionCount}
                                            </span>
                                        </div>
                                        <button
                                            type="button"
                                            ...
                                            className="inline-flex items-center justify-center px-3 py-1.5 rounded-md bg-slate-600 text-white hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition"
                                        >
                                            {pushDebugLoading ? 'Refreshing...' : 'Refresh push diagnostics'}
                                        </button>
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="bg-surface border border-border-color rounded-xl p-6 space-y-5 shadow-sm">
                            <div className="flex items-center gap-3">
                                ...
                                <h2 className="text-lg font-semibold text-text-primary">badge</h2>
                            </div>
                                ...
                                <div className="mt-3 grid grid-cols-2 gap-3">
                                    {(['light', 'dark', 'colorful', 'system'] as const).map((option) => (
                                        <button
                                            ...
                                            className={`rounded-lg border px-4 py-3 text-left transition ${theme === option
                                                    ? 'bg-primary text-white border-transparent shadow'
                                    ...
                                    <div className="mt-3 grid grid-cols-1 gap-3">
                                        {collectedBadges.map((badgeOption) => (
                                            <button
                                                ...
                                                type="button"
                                                className={`flex items-center gap-3 rounded-lg border px-4 py-3 text-left transition ${selectedBadgeId === badgeOption.id
                                ...
                                <div className="mt-3 grid grid-cols-2 gap-3">
                                    {FRAME_OPTIONS.map((frame) => (
                                        <button
                                            ...
                                            type="button"
                                            className={`rounded-lg border px-4 py-3 text-left transition ${profileFrame === frame.value
                                                    ? 'border-primary bg-primary/10 text-primary'
                                                    : 'border-border-color text-text-secondary hover:border-primary/40 hover:text-text-primary'
                                                }`}
                                        >
