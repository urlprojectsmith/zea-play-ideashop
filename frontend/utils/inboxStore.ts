...
const activityHistoryKey = 'zeaplay.activityHistory';
...
export const getActivityHistoryEntries = (): ActivityHistoryEntry[] =>
  readJson<ActivityHistoryEntry[]>(activityHistoryKey, []);

export const addActivityHistoryEntry = (entry: Omit<ActivityHistoryEntry, 'id' | 'createdAt' | 'isRead'>) => {
  const next: ActivityHistoryEntry = {
    ...entry,
    id: createId(),
    createdAt: new Date().toISOString(),
    isRead: false,
  };
  const existing = getActivityHistoryEntries();
  writeJson(activityHistoryKey, [next, ...existing]);
  return next;
};

export const markActivityHistoryRead = (ids?: string[]) => {
  const existing = getActivityHistoryEntries();
  const updated = existing.map((entry) =>
    !ids || ids.includes(entry.id) ? { ...entry, isRead: true } : entry,
  );
  writeJson(activityHistoryKey, updated);
};

export const deleteActivityHistoryEntries = (ids: string[]) => {
  const existing = getActivityHistoryEntries();
  writeJson(
    activityHistoryKey,
