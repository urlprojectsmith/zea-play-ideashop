...
const PRESENCE_HEALTH_TIMEOUT_MS = 1500;
...
const canReachPresenceServer = async (presenceUrl: string): Promise<boolean> => {
    const normalized = presenceUrl.replace(/\/$/, '');
    const controller = new AbortController();
    const timer = window.setTimeout(() => controller.abort(), PRESENCE_HEALTH_TIMEOUT_MS);
    try {
...
export const usePresence = (): PresenceState => {
    ...
    const connectSocket = useCallback(async () => {
        ...
        const isHealthy = await canReachPresenceServer(presenceUrl);
        if (!isHealthy) {
            setStatus('offline');
            setError(undefined);
            disconnectSocket();
