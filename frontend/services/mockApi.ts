...
type ApiBrowserPushSubscription = {
  id: string;
  endpoint: string;
  created_at: string;
  updated_at: string;
...
type ApiBrowserPushSettings = {
  enabled: boolean;
  vapid_public_key?: string | null;
  subscriptions: ApiBrowserPushSubscription[];
};
...
function toApiRole(role: Role | undefined): string | undefined {
  if (!role) return undefined;
  return String(role).toUpperCase();
}

function toApiUserStatus(status: UserStatus | undefined): string | undefined {
...
function mapUser(user: ApiUser): User {
  ...
  const isPresentRaw = (user as { is_present?: unknown }).is_present;
  const isPresent = typeof isPresentRaw === 'number'
    ? isPresentRaw
    : Number(isPresentRaw);
  return {
    id: user.id,
    ...
    browserNotificationsEnabled: user.browser_notifications_enabled ?? true,
    ...
    is_present: Number.isFinite(isPresent) ? isPresent : 0,
...
function normalizeApiUtcDateTime(value: string | null | undefined): string | null {
  if (!value) {
    return null;
  }
  ...
  const candidate = TIMEZONE_SUFFIX_REGEX.test(raw)
    ? raw
...
function toUtcIsoForApi(value: string | null | undefined): string | null | undefined {
  if (value === undefined) {
    return undefined;
  }
  if (value === null) {
  ...
  const fallback = TIMEZONE_SUFFIX_REGEX.test(raw)
    ? raw
    : (raw.includes('T') ? `${raw}Z` : `${raw}T00:00:00Z`);
  const reparsed = new Date(fallback);
  return Number.isNaN(reparsed.getTime()) ? raw : reparsed.toISOString();
}
...
function mapTask(task: ApiTask): Task {
  ...
  const mapped: Task = {
    ...
    dueAt: normalizeApiUtcDateTime(task.due_at),
    completedAt: normalizeApiUtcDateTime(task.completed_at),
...
function toApiTaskBackup(task: BackupTask): ApiTaskBackup {
  ...
  return {
    ...
    due_at: toUtcIsoForApi(task.dueAt) ?? null,
    completed_at: toUtcIsoForApi(task.completedAt) ?? null,
...
const api = {
  ...
  async updateUser(userId: string, updates: Partial<User>, _currentUserId: string): Promise<User> {
    try {
      ...
      if (updates.role !== undefined) body.role = toApiRole(updates.role);
      if (updates.status !== undefined) body.status = toApiUserStatus(updates.status);
      ...
      if (updates.browserNotificationsEnabled !== undefined) {
        body.browser_notifications_enabled = updates.browserNotificationsEnabled;
      }
  ...
  async updateCurrentUserProfile(userId: string, updates: Partial<Pick<User, 'name' | 'department' | 'employerId' | 'avatarAssetId' | 'avatarFrame' | 'is_present' | 'browserNotificationsEnabled'>>): Promise<User> {
    try {
      const body: Record<string, unknown> = {};
      if (updates.name !== undefined) body.name = updates.name;
      if (updates.department !== undefined) {
  ...
  async createTask(taskData: CreateTaskPayload, _creatorId: string): Promise<Task[]> {
    try {
      ...
      const payload = {
        ...
        due_at: toUtcIsoForApi(taskData.dueAt) ?? null,
    ...
    async updateTask(taskId: string, updates: UpdateTaskPayload, _updaterId: string): Promise<Task> {
      ...
      if (updates.dueAt !== undefined) body.due_at = toUtcIsoForApi(updates.dueAt);
      if (updates.completedAt !== undefined) body.completed_at = toUtcIsoForApi(updates.completedAt);
  ...
  async updateTicketTask(taskId: string, payload: TicketLinkedTaskUpdatePayload): Promise<TicketLinkedTask> {
    try {
      const { data } = await http.patch<ApiTicketLinkedTask>(`/tickets/tasks/${taskId}`, {
  ...
  async completeTicketTask(taskId: string): Promise<TicketLinkedTask> {
    try {
      const { data } = await http.post<ApiTicketLinkedTask>(`/tickets/tasks/${taskId}/complete`, {});
      invalidateCache('ticket:');
      return mapTicketLinkedTask(data);
    } catch (error) {
      mapAxiosError(error);
  ...
  async getBrowserPushSettings(): Promise<BrowserPushSettings> {
    try {
      const { data } = await http.get<ApiBrowserPushSettings>('/push/settings');
      return {
        enabled: data.enabled,
        vapidPublicKey: data.vapid_public_key ?? null,
        subscriptions: (data.subscriptions ?? []).map((item): BrowserPushSubscription => ({
          id: item.id,
  ...
  async updateBrowserPushSettings(enabled: boolean): Promise<BrowserPushSettings> {
    try {
      const { data } = await http.patch<ApiBrowserPushSettings>('/push/settings', { enabled });
      return {
        enabled: data.enabled,
        vapidPublicKey: data.vapid_public_key ?? null,
        subscriptions: (data.subscriptions ?? []).map((item): BrowserPushSubscription => ({
  ...
  async registerBrowserPushSubscription(subscription: PushSubscriptionJSON): Promise<BrowserPushSubscription> {
    try {
      ...
      const { data } = await http.post<ApiBrowserPushSubscription>('/push/subscriptions', {
  ...
  async unregisterBrowserPushSubscription(endpoint: string): Promise<void> {
    try {
      await http.delete('/push/subscriptions', { params: { endpoint } });
    } catch (error) {
      mapAxiosError(error);
    }
  },

  async sendBrowserPushTest(): Promise<{ ok: boolean; sentCount: number; message: string }> {
    try {
      const { data } = await http.post<{ ok: boolean; sent_count: number; message: string }>('/push/test', {});
      return {
        ok: Boolean(data.ok),
